...
export function downloadReciboPdf(recibo: ReciboCompleto) {
  ...
  const lines: string[] = [
    ...
    "",
    "!! RESUMEN CLAVE DEL COBRO",
    `!! Total exigible del periodo: ${formatCurrency(totalExigible)}`,
    `!! Saldo pendiente actual: ${formatCurrency(recibo.saldoPendiente)}`,
    "",
    "VI. DECLARACION",
    ...
    "",
    "Firma responsable de cobro: ____________________ || Firma de recibido del inquilino: _______________",
    SEP,
